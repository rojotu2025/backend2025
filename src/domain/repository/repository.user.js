...
const searchUserR = async (userS) => {
    try {
        ...
        if (user) {
            ...
            user.grupo = user.grupo.toUpperCase()
            user.pais = user.pais.toUpperCase()
            ...
            return {
                ...
                prendas_superiores: limites.prendas_superiores,
                prendas_inferiores: limites.prendas_inferiores,
                prendas_otros: limites.prendas_otros,
                total: limites.total,
                correo: user.correo,
...
const buscarLimitesR = async (pais, genero, rol, clima, grupo) => {
    try {
        const rols = await db.roles.findOne({
            where: {
                clima: {
                    [Op.substring]: clima
                },
