const db = require("../model/index.js");

const buscarCarritoR = async (usuario) => {
    try {
        let prendas = [];
        const carrito_id = await db.carritos.findOne({
            where: {
                usuario: usuario,
            }
        });

        const carrito = await db.carrito_prendas.findAll({
            where: {
                id_carrito : carrito_id.id
            }
        })

        for (let index = 0; index < carrito.length; index++) {
            let prenda = {}
            prenda = await db.prendas.findOne({
                where:{id: carrito[index].dataValues.id_prenda}
            })
            const id_prenda_carrito = carrito[0].dataValues.id_prenda_carrito
            prendas.push({...prenda.dataValues, id_prenda_carrito })
        }
        
        return prendas
    } catch (error) {
        return false;
    }
}

const agregarPrendaR = async (usuario, id_prenda, id_prenda_carrito, talla) => {
    const date = new Date();
    try {
        let id;
        const carrito_id = await db.carritos.findOne({
            where: {
                usuario: usuario,
            }
        });
        
        if(!carrito_id){
        const carrito = await db.carritos.create(
            {
                usuario: usuario,
                estado_carrito: "creado",
                fecha_solicitud: date.getTime(),
                fecha_actualizacion: date.getTime()
            });
            id=carrito.id
        }else{
            id=carrito_id.id
        }

        const carrito_prendas = await db.carrito_prendas.create(
            {
                id_carrito: id,
                id_prenda: id_prenda,
                id_prenda_carrito: id_prenda_carrito,
                talla : talla
            });

        if (carrito_prendas) {
            return true
        } else {
            return false
        }
    } catch (error) {
        return false;
    }
}

const quitarPrendaR = async (usuario, id_prenda, id_prenda_carrito) => {
    try {
        const carrito_id = await db.carritos.findOne({
            where: {
                usuario: usuario,
            }
        });
        return await db.carrito_prendas.destroy({
            where: {
                id_carrito : carrito_id.id,
                id_prenda: id_prenda,
                id_prenda_carrito : id_prenda_carrito
            }
        });
    } catch (error) {
        return false;
    }
}

module.exports = { buscarCarritoR, agregarPrendaR, quitarPrendaR  }