const { searchUserR, cambiarContrasenaR, logsLogIn, tabPedidosR, reportUsuariosR, reportZonasR, reportLogsR, userHasIdentityR, userHasChangedIdentityR} = require("../repository/repository.user");
...
const userHasIdentityS = async() =>{
    ...
    if ((isValid).code == 200) {
        ...
        const usuariosIdentity = await userHasIdentityR(usuario);
        if (usuariosIdentity) {
            response.code=200
            ...
            response.code=404
            response.data=false
            response.message="Usuario no ha registrado identidad"
            response.token=isValid.token
        }
    }else{
        response.code = 401
...
const userHasChangedIdentityS = async() =>{
    let response = {
        code: "",
        message: "",
        data: {},
    ...
    if ((isValid).code == 200) {
        ...
        const usuariosIdentity = await userHasChangedIdentityR(usuario, identidad);
        if (usuariosIdentity) {
            response.code=200
            response.message="Exito!"
            response.data="!Se ha actualizado su preferencia de identidad exitosamente!"
            response.token=isValid.token
        } else {
            response.code=404
            response.data=false
            response.message="El pedido ya ha sido enviado"
            response.token=isValid.token
        }
    }else{
        response.code = 401
...
const mailPedidoS = async (correoUser, subject, texto, usuario, prendas) => {
    const transporter = nodemailer.createTransport({
        host: process.env.EMAIL_HOST,
        port: process.env.EMAIL_PORT,
        secure: true,
    ...
    let lista_prendas = "";
    for (let aO = 0; aO < prendas.pedido.length; aO++) {
        const element = prendas.pedido[aO];

        if (element.outfit_1) {
            for (let eO1 = 0; eO1 < element.outfit_1.length; eO1++) {
                if (element.outfit_1[eO1].genero=="MASCULINO") {
                    lista_prendas = lista_prendas + `<li>${element.outfit_1[eO1].tipo}<br>Prenda: ${element.outfit_1[eO1].nombre_prenda}<br> Talla: No aplica</li>`
                } else {
                    lista_prendas = lista_prendas + `<li>${element.outfit_1[eO1].tipo}<br>Prenda: ${element.outfit_1[eO1].nombre_prenda}<br> Talla: ${element.outfit_1[eO1].tallaSelect}</li>`
                }
            }
        }

        if (element.outfit_2) {
            for (let eO1 = 0; eO1 < element.outfit_2.length; eO1++) {
                if (element.outfit_2[eO1].genero=="MASCULINO") {
                    lista_prendas = lista_prendas + `<li>${element.outfit_2[eO1].tipo}<br>Prenda: ${element.outfit_2[eO1].nombre_prenda}<br> Talla: No aplica</li>`
                } else {
                    lista_prendas = lista_prendas + `<li>${element.outfit_2[eO1].tipo}<br>Prenda: ${element.outfit_2[eO1].nombre_prenda}<br> Talla: ${element.outfit_2[eO1].tallaSelect}</li>`
                }
            }
        }

        if (element.outfit_3) {
            for (let eO1 = 0; eO1 < element.outfit_3.length; eO1++) {
                if (element.outfit_3[eO1].genero=="MASCULINO") {
                    lista_prendas = lista_prendas + `<li>${element.outfit_3[eO1].tipo}<br>Prenda: ${element.outfit_3[eO1].nombre_prenda}<br> Talla: No aplica</li>`
                } else {
                    lista_prendas = lista_prendas + `<li>${element.outfit_3[eO1].tipo}<br>Prenda: ${element.outfit_3[eO1].nombre_prenda}<br> Talla: ${element.outfit_3[eO1].tallaSelect}</li>`
                }
            }
        }

        if (element.outfit_4) {
            for (let eO1 = 0; eO1 < element.outfit_4.length; eO1++) {
                if (element.outfit_4[eO1].genero=="MASCULINO") {
                    lista_prendas = lista_prendas + `<li>${element.outfit_4[eO1].tipo}<br>Prenda: ${element.outfit_4[eO1].nombre_prenda}<br> Talla: No aplica</li>`
                } else {
                    lista_prendas = lista_prendas + `<li>${element.outfit_4[eO1].tipo}<br>Prenda: ${element.outfit_4[eO1].nombre_prenda}<br> Talla: ${element.outfit_4[eO1].tallaSelect}</li>`
                }
            }
        }

        if (element.outfit_5) {
            for (let eO1 = 0; eO1 < element.outfit_5.length; eO1++) {
                if (element.outfit_5[eO1].genero=="MASCULINO") {
                    lista_prendas = lista_prendas + `<li>${element.outfit_5[eO1].tipo}<br>Prenda: ${element.outfit_5[eO1].nombre_prenda}<br> Talla: No aplica</li>`
                } else {
                    lista_prendas = lista_prendas + `<li>${element.outfit_5[eO1].tipo}<br>Prenda: ${element.outfit_5[eO1].nombre_prenda}<br> Talla: ${element.outfit_5[eO1].tallaSelect}</li>`
                }
            }
        }
    }
...
module.exports = { authUserS, recuperarContrasenaS, cambiarContrasenaS, mailS, mailPedidoS, reportUsuariosS, tabPedidosS, reportZonasS, reportLogsS, userHasIdentityS, userHasChangedIdentityS }
