...
const agregarPrendaS = async (req, id_prenda, talla, cantidad) => {
    let response = {
        code: "",
        message: "",
        data: ""
    ...
    if ((isValid).code == 200) {
        ...
        const prenda = await agregarPrendaR(d.usuario , id_prenda, id_prenda_carrito, talla, cantidad);
        if (prenda) {
            response.code = 200
            response.message = "autorizado"
...
const quitarPrendaS = async (req, id_prenda_carrito) => {
    let response = {
        code: "",
        message: "",
        data: ""
    ...
    if ((isValid).code == 200) {
        const usuario = isValid;
        const pedido = await quitarPrendaR(d.usuario, id_prenda_carrito);
        if (pedido) {
            response.code = 200
            response.message = "autorizado"
            response.data = "Exito"
