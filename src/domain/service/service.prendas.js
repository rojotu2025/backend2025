...
const listarPrendasS = async (token, sexo, rol, clima, grupo, pais, identidad) => {
    let response = {
        code: "",
        message: "",
        data: ""
    ...
    if ((await isValid).code == 200) {
        ...
        let newPrendas = {}
        if(rol=="BACK"){
            let prendas = await listarPrendasR(sexo, rol, clima, grupo, pais, identidad);
            if (prendas) {
                let superiores = [];
                let inferiores = [];
                let otros = [];
            ...
            const prendas = await listarPrendasFrontR(sexo, rol, clima, grupo, pais, identidad);
            if (prendas) {
                response.code = 200;
                response.message = "autorizado";
...
const listarPrendasFrontS = async (token, sexo, rol, clima, grupo, pais, identidad) => {
    let response = {
        code: "",
        message: "",
        data: ""
